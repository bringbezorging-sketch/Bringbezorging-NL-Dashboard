...
// In-memory users (admins can add more)
...
export function detectFraud(o) {
  ...
  if (gw.includes('cash')||gw.includes('cod')||gw.includes('contant')) { reasons.push('Contant betaald'); score+=3; }
...
export function transformOrder(o) {
  ...
  let payment = o.payment_gateway||'Onbekend';
  if (payment.toLowerCase().includes('ideal')) payment='iDEAL';
  else if (payment.toLowerCase().includes('cash')||payment.toLowerCase().includes('cod')) payment='Contant';
  return {
    id: `#${o.order_number}`,
    shopifyId: o.id,
