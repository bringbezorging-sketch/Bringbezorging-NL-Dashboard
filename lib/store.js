...
export function detectFraud(o) {
  ...
  // Betaald via iDEAL — nooit verdacht
  ...
  const isContant = o.financial_status === 'pending';
  const total = parseFloat(o.total_price || 0);
  const email = (o.email || '').toLowerCase();
  const addr = o.shipping_address?.address1 || '';
  const noHouseNr = addr.length > 0 && !/\d/.test(addr);

  // Alleen verdacht als contant + eerste bestelling
  if (isContant && isFirstOrder) { reasons.push('Eerste contante bestelling'); score += 4; }
  
  // Extra risico bij hoog bedrag
  if (total >= 40 && isContant && isFirstOrder) { reasons.push(`Hoog bedrag (€${total.toFixed(2)})`); score += 2; }
  ...
  // Geen huisnummer — alleen als waarschuwing, niet als fraudescore
  if (noHouseNr) reasons.push('Controleer adres: geen huisnummer');
